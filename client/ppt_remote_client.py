...
def listen_for_keys():
    ...
    ppt_thread = win32com.client.Dispatch("PowerPoint.Application")  # New COM object in this thread

    def is_ppt_running():
        try:
            return ppt_thread.SlideShowWindows.Count > 0
        except Exception as e:
            print(f"[CHECK ERROR - thread] {e}")
            return False

    def on_key(event):
        print(f"[KEY] {event.name}")
        if is_ppt_running():
            print(f"[SEND CMD] {event.name}")
            if event.name == 'left':
                send_command('PREV')
                get_server_slide()
            ...
            print("[SKIP] PowerPoint not in slideshow")
